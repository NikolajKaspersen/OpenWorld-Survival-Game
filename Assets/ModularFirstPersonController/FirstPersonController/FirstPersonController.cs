...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //Disables cursor lock when we are in pause screen
        HandleCursorLock();
        //Disables crosshair while we are in pause screen
        UpdateCrosshair();
    }

    private void HandleCursorLock()
    {
        if (!PauseMenu.isPaused && !EventSystem.current.IsPointerOverGameObject())
        {
            // If the game is not paused and the pointer is not over a UI element, lock the cursor
            if (lockCursor)
            {
                Cursor.lockState = CursorLockMode.Locked;
        ...
        {
            // If the game is paused or the pointer is over a UI element, release the cursor
    ...
    {
        // Check if the crosshair is enabled
        if (crosshair)
        {
            if (!PauseMenu.isPaused)
            {
                // If the game is not paused, display the crosshair
                crosshairObject.sprite = crosshairImage;
                crosshairObject.color = crosshairColor;
            ...
            {
                // If the game is paused, hide the crosshair
                crosshairObject.gameObject.SetActive(false);
            }
        }
    }
